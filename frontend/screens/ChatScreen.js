...
//       );
//       if (proposalMsg) {
//         const ownerFromMsg = proposalMsg.ownerId || proposalMsg.to;
//         setProposalOwnerId(ownerFromMsg);

//         // Debug: inspect the proposal message and owner resolution
//         console.log("proposalMsg loaded:", {
//           id: proposalMsg.id,
//           ownerId: proposalMsg.ownerId,
//           to: proposalMsg.to,
//           resolvedOwner: ownerFromMsg,
//         });
...
//     return () => unsub();
//   }, [chatId, currentItem, fetchedItem, me]);
...
//       snap.forEach((d) => {
//         const msg = d.data();
//         if (msg.to === me.uid && msg.status !== "seen" && msg.type !== "rental_proposal") {
...
//  // ✅ Send rental proposal + trigger email notification
// // ✅ Send rental proposal + trigger email notification (robust logging + error handling)
...
//   try {
//     // 1) Write proposal message to Firestore
//     const msgDocRef = await addDoc(collection(db, "chats", chatId, "messages"), {
//       type: "rental_proposal",
//       senderId: me.uid,
//       to: otherId,
//       ownerId: currentItem?.ownerId || otherId, // helpful fallback on message
//       itemId,
//       itemTitle,
...
//     // 2) Trigger Cloud Function to send email notification (await and inspect response)
//     const payload = {
//       recipientId: otherId,
//       // include recipientEmail optionally if you want to test directly
//       senderId: me.uid,
//       senderName: me.displayName || me.email || "Someone",
//       itemTitle,
//       startDate: startDate.toISOString(),
//       endDate: endDate.toISOString(),
//       text: `Rental proposal for "${itemTitle}" (${readableStart} → ${readableEnd})`,
...
//     console.log("notifyProposalEmail: calling function with payload:", payload);

//     // Ensure BASE_URL has no trailing slash (you already set it to cloudfunctions.net)
//     const url = `${BASE_URL}/notifyProposalEmail`;

//     // await the fetch so we can see the exact status & body in logs
...
//     const respText = await resp.text().catch(() => "<non-text response>");
//     console.log("notifyProposalEmail response status:", resp.status, "body:", respText);

//     if (!resp.ok) {
//       // show actionable feedback to developer / user
//       console.warn("notifyProposalEmail returned non-OK:", resp.status, respText);
//       // optional: mark message with a flag (not required)
//       // await updateDoc(msgDocRef, { emailNotify: "failed", emailStatusCode: resp.status });
//       Alert.alert("Notice", "Proposal sent but notification failed (see logs).");
//     } else {
//       // success
//       // await updateDoc(msgDocRef, { emailNotify: "sent" });
//       console.log("notifyProposalEmail OK");
//     }
//   } catch (err) {
//     console.error("sendRentalProposal error:", err);
//     Alert.alert("Error", "Failed to send proposal. Check logs.");
//   } finally {
//     // 3) Close date picker (always)
//     setShowPicker(false);
//   }
// };
...
//         // Robust owner detection: prefer explicit ownerId saved on message
        

        

//       // 🟢 Robust ownership identification
//       const msgOwnerId =
//         m.ownerId ||
//         m.to ||
//         proposalOwnerId ||
...
//       const isMeOwner = !!me && me.uid === msgOwnerId;

//       // Show actions if proposal is still actionable.
// // Accept either pending (new) or seen (older messages already flagged by markSeen).
// const showOwnerActions = (m.status === "pending" || m.status === "seen") && isMeOwner && !mine;


//       // DEBUG: helpful console logging to see why actions may be hidden later
//         // (remove these logs after you confirm behavior)
//         console.log("rental_proposal render:", {
//           msgId: m.id,
//           senderId: m.senderId,
//           to: m.to,
//           ownerId_on_msg: m.ownerId,
//           resolvedOwner: msgOwnerId,
//           meUid: me?.uid,
//           isMeOwner,
//           status: m.status,
//           showOwnerActions,
...
export default function ChatScreen({ route, navigation }) {
  ...
  const sendMessage = async () => {
    if (!text.trim() || !me || !otherId || !chatId) return;
    const trimmed = text.trim();
    await addDoc(collection(db, "chats", chatId, "messages"), {
      text: trimmed,
      senderId: me.uid,
  ...
  // Send rental proposal + email
  const sendRentalProposal = async () => {
    if (!itemId)
      return Alert.alert("Error", "This chat has no associated item.");
    if (endDate < startDate)
      return Alert.alert("Error", "End date cannot be before start date.");
    if (!me || !otherId || !chatId)
      return Alert.alert("Error", "Missing user or chat context.");
    ...
    try {
      await addDoc(collection(db, "chats", chatId, "messages"), {
        ...
        senderId: me.uid,
        to: otherId,
        ownerId: currentItem?.ownerId || otherId,
        itemId,
        itemTitle,
        itemImage: item?.imageUrl || null,
        startDate: startDate.toISOString(),
      ...
      const respText = await resp.text().catch(() => "<non-text response>");
      console.log(
        "notifyProposalEmail response status:",
        resp.status,
        "body:",
        respText
      );
      ...
      if (!resp.ok) {
        console.warn(
          "notifyProposalEmail returned non-OK:",
          resp.status,
          respText
        );
        Alert.alert(
          "Notice",
          "Proposal sent but notification failed (see logs)."
        );
      } else {
        console.log("notifyProposalEmail OK");
      }
  ...
  const renderMessage = ({ item: m }) => {
    ...
    if (m.type === "rental_proposal") {
      ...
      const showOwnerActions =
        (m.status === "pending" || m.status === "seen") &&
        isMeOwner &&
        !mine;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Borrower Proposal Button + Date pickers */}
        {!isOwner &&
          itemId &&
          (!showPicker ? (
            <TouchableOpacity
